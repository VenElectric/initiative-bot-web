  ...
  useEffect(() => {
    ...
    socket.on("client_del_spell", function (data: any) {
      ...
      setTimeout(() => {
		...
		let newState = [] as InitiativeLine[];
                    ...
                    ? init_list.map((item: InitiativeLine) => {
                        return (
                          <Alert
                            key={item.id}
                            style={{
                              borderStyle: `${item.cmark ? "solid" : "none"}`,
                              borderColor: `${
                                item.cmark ? "#33ff00" : "black"
                              }`,
                              borderWidth: "3px",
                            }}
                          >
                            <Accordion className="initrecord">
                              <InitRecord init_rec={item} />
                            </Accordion>
                          </Alert>
                        );
                      })
